$(".btnsub").submit(function () {
  ...
  if ($(".q7:checked").length == "") {
    alert("7.ปัจจุบันท่านออกกำลังกายโดยเฉลี่ยสัปดาห์ละกี่วัน");
    $("#checkq7-1").focus();
    return false;
  }
  if ($(".q8:checked").length == "") {
    alert("8.ปัจจุบันท่านสูบบุหรี่/บุหรี่ไฟฟ้า/ใบจาก/ยาเส้น หรือไม่");
    $("#checkq8-1").focus();
    return false;
  }
  if ($(".q9:checked").length == "") {
    alert("9.ปัจจุบันท่านดื่มเครื่องดื่มที่มีแอลกอฮอล์");
    $("#checkq9-1").focus();
    return false;
  }
  if ($(".q10:checked").length == "") {
    alert("10. โดยรวมแล้วท่านพึงพอใจกับสุขภาพกายของท่าน เพียงใด");
    $("#checkq10-1").focus();
    return false;
  }
